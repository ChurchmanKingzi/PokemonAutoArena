/**
 * Team management system with Pokémon integration
 */
...
/**
 * Updates the team slots based on current configuration - with searchable Pokemon dropdown
 */
    ...
    for (let i = 0; i < teamCount; i++) {
        ...
        for (let j = 0; j < 6; j++) {
            ...
            if (j < fightersPerTeam) {
                ...
                // Add sprite if Pokemon is selected
...
function createPokemonStatsTooltip(character) {
    ...
    let content = `
        ...
        <div class="tooltip-section">
            <div class="tooltip-row"><span class="tooltip-label">GENA:</span> <span class="tooltip-value">${character.combatStats.gena}</span></div>
            <div class="tooltip-row"><span class="tooltip-label">PA:</span> <span class="tooltip-value">${character.combatStats.pa}</span></div>
    ...
    if (character.originalKP) {
        ...
        content += `<div class="tooltip-row"><span class="tooltip-label">KP:</span> <span class="tooltip-value">${character.combatStats.kp}</span></div>`;
    }
    ...
    if (Object.keys(statsGerman).length > 0) {
        for (const [statName, statValue] of Object.entries(statsGerman)) {
            ...
            if (statName === 'Initiative') {
                content += `<div class="tooltip-row"><span class="tooltip-label">${statName}:</span> <span class="tooltip-value">${statValue} (${initiativeDisplay})</span></div>`;
            } else {
                content += `<div class="tooltip-row"><span class="tooltip-label">${statName}:</span> <span class="tooltip-value">${statValue}</span></div>`;
            }
        }
        ...
        content += `
            <div class="tooltip-row"><span class="tooltip-label">Initiative:</span> <span class="tooltip-value">${initiativeBase} (${initiativeDisplay})</span></div>
    ...
    // Add extra CSS style for tooltip rows and terrain attributes
    content = `
        <style>
