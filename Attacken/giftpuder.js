/**
 * Giftpuder (Poison Powder) attack implementation
 * Applies poison status effect in a cone
 */
...
/**
 * GiftpuderAttack class for handling the poison powder attack
 */
export class GiftpuderAttack {
    /**
     * Create a new Giftpuder attack
    ...
    constructor(attacker, target, attack, isHit, callback, activeProjectilesArray) {
        ...
        this.statusTimeout = setTimeout(() => this.applyPoisonStatus(), EFFECT_DURATION - 100);
    ...
    createPowderParticles() {
        ...
        particleContainer.className = 'giftpuder-particles-container';
        ...
        for (let i = 0; i < POWDER_PARTICLE_COUNT; i++) {
            ...
            particle.className = 'giftpuder-particle';
            ...
            // Purple color with random opacity
            const opacity = 0.4 + Math.random() * 0.6;
            particle.style.backgroundColor = `rgba(175, 106, 175, ${opacity})`;
            particle.style.borderRadius = '50%';
            
            // Add animation
            particle.style.animation = `giftpuderFloat ${0.8 + Math.random() * 1.5}s ease-in-out infinite alternate`;
    ...
    /**
     * Apply a purple flash effect to a target Pokémon
     * @param {string} targetId - Character ID of the target
     */
    applyPurpleFlashEffect(targetId) {
        ...
        characterEls.forEach(charEl => {
            // Add the poison flash class
            charEl.classList.add('poison-flash-effect');
            
            // Remove the class after 500ms (half a second)
            setTimeout(() => {
                charEl.classList.remove('poison-flash-effect');
            }, 500);
        });
    }
    ...
    destroy() {
        ...
        if (this.callback) {
            try {
                ...
                console.error('Error in Giftpuder callback:', error);
            }
        }
    }
}
 ...
 * @returns {GiftpuderAttack} - The created attack
 */
export function createGiftpuder(attacker, target, attack, isHit, callback, activeProjectiles) {
    return new GiftpuderAttack(attacker, target, attack, isHit, callback, activeProjectiles);
}

/**
 * Add CSS styles for Giftpuder
 */
export function addGiftpuderStyles() {
    // Check if styles already exist
    if (document.getElementById('giftpuder-styles')) return;
    
    // Create style element
    const styleElement = document.createElement('style');
    styleElement.id = 'giftpuder-styles';
    
    // Define the CSS
    styleElement.textContent = `
        /* Giftpuder particle styling */
        .giftpuder-particle {
            position: absolute;
            background-color: rgba(175, 106, 175, 0.7);
            border-radius: 50%;
            box-shadow: 0 0 4px rgba(175, 106, 175, 0.5);
            pointer-events: none;
        }
        ...
        /* Purple flash effect for poisoned Pokemon */
        @keyframes purpleFlash {
            0% { filter: brightness(1) saturate(1); }
            50% { filter: brightness(1.5) saturate(1) drop-shadow(0 0 5px rgba(175, 106, 175, 0.9)) hue-rotate(260deg); }
            100% { filter: brightness(1) saturate(1); }
        }
        
        /* Class to apply the purple flash effect */
        .poison-flash-effect {
            animation: purpleFlash 0.5s ease-in-out forwards;
        }
    `;
